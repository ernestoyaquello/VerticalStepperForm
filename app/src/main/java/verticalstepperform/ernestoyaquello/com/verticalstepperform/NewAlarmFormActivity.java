...
public class NewAlarmFormActivity extends VerticalStepperFormBaseActivity {
    ...
    @Override
    protected View addCustomStep(int numStep, RelativeLayout stepContent) {
        ...
        switch (numStep) {
            case TITLE_STEP_NUM:
                view = addAlarmTitleStep(stepContent);
                break;
            case DESCRIPTION_STEP_NUM:
                view = addAlarmDescriptionStep(stepContent);
                break;
            case TIME_STEP_NUM:
                view = addAlarmTimeStep(stepContent);
                break;
            case DAYS_STEP_NUM:
                view = addAlarmDaysStep(stepContent);
                break;
        }
        return view;
    }
    ...
    private View addAlarmTitleStep(RelativeLayout stepContent) {
        // This step is added to the form generating its only view programmatically
    ...
    private View addAlarmDescriptionStep(RelativeLayout stepContent) {
        // This step is added to the form generating its only view programmatically
    ...
    private View addAlarmTimeStep(RelativeLayout stepContent) {
        // This step is added to the form inflating a layout XML file
        LayoutInflater inflater = LayoutInflater.from(getBaseContext());
        LinearLayout timeStepContent =
                (LinearLayout) inflater.inflate(R.layout.step_time_layout, null, false);
        // In this case we add the step view manually as we need to use it now
        stepContent.addView(timeStepContent);
        timeTextView = (TextView) stepContent.findViewById(R.id.time);
        ...
        // The view has already been added manually, so we return null here
        return null;
    }

    // Creation of the week days step
    private View addAlarmDaysStep(RelativeLayout stepContent) {
        // This step is added to the form inflating a layout XML file
        LayoutInflater inflater = LayoutInflater.from(getBaseContext());
